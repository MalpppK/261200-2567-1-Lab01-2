...
public class Turtle implements Runnable, ActionListener, MouseListener, MouseMotionListener, KeyListener,
    ...
    public static void main(String[] a) {
        ...
        // make pride month by bob I
        Turtle bub = new Turtle();
        bub.width(5);
        bub.speed(2);
        bub.penColor("green");
        bub.up();
        ...
        // make pride month by bup D
        Turtle bup = new Turtle();
        bup.width(5);
        bup.speed(2);
        bup.penColor("yellow");
        bup.up();
        ...
        // make pride month by bu E
        Turtle bu = new Turtle();
        bu.width(5);
        bu.speed(2);
        bu.penColor("Red");
        bu.up();
    ...
    private static void makeShapes() {
        ...
        int[] xs = new int[] { 66, 65, 63, 61, 53, 44, 33, 24, 23, 19, 17, 14, 9, 8, 8, 10, 13, 11, 10, 2, 9, 11, 15,
                11, 11, 10, 12, 18, 20, 22, 23, 26, 35, 44, 53, 61, 62, 64, 66, 71, 77, 78, 77, 76, 72, 77, 81, 86, 91,
                94, 97, 98, 97, 95, 92, 87, 82, 77, 72, 74, 77, 78, 76, 70 };
        int[] ys = new int[] { 18, 19, 21, 25, 23, 22, 23, 27, 25, 21, 20, 21, 27, 30, 32, 34, 37, 42, 47, 50, 53, 59,
                65, 68, 69, 71, 74, 79, 80, 80, 78, 74, 77, 78, 77, 75, 79, 81, 82, 81, 76, 73, 71, 69, 66, 59, 60, 61,
                60, 58, 54, 50, 46, 42, 40, 39, 40, 41, 34, 32, 28, 27, 24, 19 };
    ...
    private long storeCurrentState(boolean animate, boolean isStamp, double dotSize, Color dotColor, Font font,
            String text, int justification, Point2D.Double textOffset) {
     ...
     * 0(default)=Animated (The turtle will slide from one state to another without
     * being jerky.)
     * 1=State Change (The turtle will refresh immediately to the last state. Jerky
     * motion.)
            ...
            for (Method m : Class.forName(className).getDeclaredMethods()) {
                if (m.getName().equals(methodName)) {
                    ...
                    for (Class paramType : m.getParameterTypes()) {
                        ...
                        if (!paramType.getName().equals("double") && !paramType.getName().equals("java.lang.Double")
        ...
        synchronized (turtleLock) {
            ...
            if (turtleStates.lastKey() > time && getStateSpeed(turtleStates.lastEntry().getValue()) > 0) {
                double percent = 1 - (turtleStates.lastKey() - time)
                        / getStateSpeed(turtleStates.lastEntry().getValue()) / 1000000.0;
        ...
        synchronized (turtleLock) {
            ...
            if (renderTime < turtleStates.lastKey()) {
                ...
                if (animatedTurtle._speed > 0) {
                    percent = 1
                            - (turtleStates.ceilingKey(renderTime) - renderTime) / animatedTurtle._speed / 1000000.0;
                } else
                    percent = 1;
                if (percent < 0)
                    percent = 0;
    ...
    private void drawLine(double percent, Graphics2D g) {
        ...
        if (__location != null && !__location.equals(_location)) {
            ...
            g.drawLine((int) ((x1 - centerX) * scale + width / 2), (int) ((y1 - centerY) * (-scale) + height / 2),
                    (int) ((x2 - centerX) * scale + width / 2), (int) ((y2 - centerY) * (-scale) + height / 2));
        }
    }
    ...
    private void drawStamp(double percent, Graphics2D g, boolean isStamp) {
        ...
        if (_image == null) {
            ...
            if (_outlineWidth > 0) {
                g.setStroke(new BasicStroke((float) (_outlineWidth * scale), BasicStroke.CAP_ROUND,
    ...
    private static void drawBackground(Graphics2D g) {
        ...
        if (backgroundMode == BACKGROUND_MODE_CENTER) {
            ...
            offscreen.drawImage(backgroundImage, (int) ((-w / 2 - centerX) * scale + width / 2),
                    (int) ((h / 2 - centerY) * (-scale) + height / 2), (int) (w * scale), (int) (h * scale), null);
            ...
            for (double x = ((int) (left / w) - 1) * w; x <= right; x += w)
                for (double y = ((int) (bottom / h)) * h; y <= top + h; y += h)
                    offscreen.drawImage(backgroundImage, (int) ((x - centerX) * scale + width / 2),
                            (int) ((y - centerY) * (-scale) + height / 2), (int) Math.ceil(w * scale),
    ...
    private void drawCrossHairs(double percent, Graphics2D g) {
        ...
        r += f;
        for (int i = 0; i < 4; i++)
            g.drawLine((int) (r * Math.cos(Math.PI / 2 * i + 2 * Math.PI * time / period)),
                    (int) (r * Math.sin(Math.PI / 2 * i + 2 * Math.PI * time / period)),
                    (int) ((r + r / 5) * Math.cos(Math.PI / 2 * i + 2 * Math.PI * time / period)),
                    (int) ((r + r / 5) * Math.sin(Math.PI / 2 * i + 2 * Math.PI * time / period)));
        r -= f;
    ...
    public void actionPerformed(ActionEvent e) {
        if (((JMenuItem) e.getSource()).getText().equals("Save...")) {
            ...
            chooser.setFileFilter(new FileNameExtensionFilter("Image (*.jpg, *.jpeg, *.gif, *.bmp, *.png)", "jpg",
                    "png", "jpeg", "bmp", "gif"));
